...
var activadopostular = false;
	...
	function postulacionfinal(){
			...
			window.parent.fnCierraFormularioModificado(<?php echo $i; ?>,1);
	}
  ...
  reader.onload = function(event){
    ...
    $.ajax({
      ...
      dataType: 'text'
        ...
        console.log('--->pages/guarda_archivovivienda_mod.php: '+data);
		$("#cuadro_guardar").hide();
		$("#"+viene).next().remove();
		if(data == "-1"){
			mataFile(viene);
...
if(paso==4 || paso==3)
{
  ...
  if (window.console) console.log('--->pages/guarda_vivienda_mod.php?'+$("#paso"+paso).serialize()+"&i="+llave+"&v="+vivienda);
  $.when(
  $.ajax({
		type: "GET",
		url: "pages/guarda_vivienda_mod.php",
		data: dataString,
		async: false,
		success: function (data) {
			if (window.console) console.log('--->DEBUG guarda_vivienda_mod: '+data);
			if (data == "2") {
				flag = true;
			}			
		},
...
if(paso==1 || paso==2)
{
  ...
  if (window.console) console.log('--->pages/guarda_vivienda_mod.php?'+$("#paso"+paso).serialize()+"&i="+llave);
  $.when(
  $.ajax({
		type: "GET",
		url: "pages/guarda_vivienda_mod.php",
...
if(paso==4)
{
 ...
 if(!$("#condiciones").is(':checked')) 
 {
  ...
  if (window.console) console.log('--->pages/guarda_vivienda_mod.php?'+$("#paso"+paso).serialize());
...
if(paso==3)
{
 ...
 $(".guardar").html("<br>Estamos Guardando Automáticamente su Información.<br><img src='img/load.gif'>");
  ...
  if (window.console) console.log('--->pages/guarda_vivienda_mod.php?'+dataString);
 ...
 //maxDate: fecha tope D=días | M=mes | Y=año
	...
	$(".botonenviarpostulacion").click(function(){
		...
		if(valida_postulacion_contador(1)){  
			...
			$(".actionBar").append('<a href="#" class="postular_final" onclick="postulacionfinal()" title="" style="float: left; margin-left: 180px; background-color: #009933; color: white; ">ENVIAR CORRECCIONES</a>');
