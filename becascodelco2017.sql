...
AS
	select `POSTULACIONES_WEB`.`IDPOSTULACION` AS `IDPOSTULACION`,concat('k=1&r=',`POSTULACIONES_WEB`.`RUT_TRABAJADOR`,'&dv=',`POSTULACIONES_WEB`.`DV_TRABAJADOR`,'&i=',`POSTULACIONES_WEB`.`IDPOSTULACION`) AS `data1`,concat('http://www.oticdelaconstruccion.cl/fondos2016/estudios_open.php?',`BASE64_ENCODE`(concat('k=1&r=',`POSTULACIONES_WEB`.`RUT_TRABAJADOR`,'&dv=',`POSTULACIONES_WEB`.`DV_TRABAJADOR`,'&i=',`POSTULACIONES_WEB`.`IDPOSTULACION`))) AS `link_me` from (`POSTULACIONES_WEB` join `USUARIOS_SINDOC_NABARCA` on((`POSTULACIONES_WEB`.`IDPOSTULACION` = `USUARIOS_SINDOC_NABARCA`.`id`)));
...
AS
	select `POSTULACIONES_WEB`.`IDPOSTULACION` AS `IDPOSTULACION`,`SPLIT_STR`(`RESULTADOS_ESTUDIOS_2016`.`RUT_T`,'-',1) AS `RUT_TRABAJADOR`,`RESULTADOS_ESTUDIOS_2016`.`RUT_T` AS `RUT_T`,`RESULTADOS_ESTUDIOS_2016`.`RUT_P` AS `RUT_P`,`RESULTADOS_ESTUDIOS_2016`.`ESTADO_ADJ` AS `ESTADO_ADJ`,`POSTULACIONES_WEB`.`FECHA_CREACION` AS `FECHA_CREACION` from (`RESULTADOS_ESTUDIOS_2016` join `POSTULACIONES_WEB` on((`RESULTADOS_ESTUDIOS_2016`.`IDPOST` = `POSTULACIONES_WEB`.`IDPOSTULACION`))) where (`POSTULACIONES_WEB`.`IDTIPOBECA` = 2);
