...
if(isset($_POST['email_convo'])) {
	$fetch = mysql_query("SELECT * FROM users WHERE username = '".$_SESSION['username']."' ");
	$result = mysql_fetch_array($fetch);
	$query = mysql_query("SELECT * FROM archive WHERE convoID = '".$_POST['convoID']."' ");
...
if(!empty($_GET['delete'])) {
	$fetch = mysql_query("SELECT * FROM leads WHERE id = '".$_GET['delete']."' ");
	$fetch_result = mysql_fetch_array($fetch);
	mysql_query("DELETE FROM leads WHERE id = '".$_GET['delete']."' ");
	mysql_query("DELETE FROM archive WHERE convoID = '".$fetch_result['transcript']."' ");
}

// build array of leads
$leads = array();
$count = 0;
	$query = mysql_query("SELECT * FROM leads ORDER BY id DESC");
		while ($row= mysql_fetch_array($query)) {
