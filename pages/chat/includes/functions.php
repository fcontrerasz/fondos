...
global $archive_path;
$userInfo = mysql_query("SELECT * FROM sessions WHERE convoID = '".$convoID."' ");
$result = mysql_fetch_array($userInfo);
	...
	$trans = mysql_query("SELECT * FROM transcript WHERE convoID = '".$convoID."' ORDER BY id ASC ");
...
function archive($convoID,$name,$email) {
	include "date.php";
	$check = mysql_query("SELECT * FROM archive WHERE convoID = '".$convoID."' ");
	if(mysql_num_rows($check) == 0) {
		$scrape = mysql_query("SELECT * FROM transcript WHERE convoID = '".$convoID."' ORDER BY id ASC ");
		while($row = mysql_fetch_array($scrape)) {
			mysql_query("INSERT INTO archive (name,message,user,convoID,time,class) 
			VALUES
			('".$row['name']."','".$row['message']."','".$row['user']."','".$row['convoID']."','".$row['time']."','".$row['class']."')
			");
		}
		mysql_query("INSERT INTO leads (name,email,transcript,date)
                VALUES
