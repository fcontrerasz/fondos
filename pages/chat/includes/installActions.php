...
// step 2 - check permissions
	...
	echo '<h3>Checking file permissions</h3>';
		...
		$filesFile = "../files/writeTest.txt";
		if(is_writable($includesFile)) {
			echo '<div class="success"><p>Includes directory is writeable!</div>';
		} else {
			echo '<div class="error"><p>Includes directory is NOT writeable!</div>';		
			$errors = 1;
		}
		if(is_writable($archiveFile)) {
                        ...
                        $errors = 1;
			echo '<div class="error"><p>Archive directory is not not writeable!</div>';
                }
		if(is_writable($filesFile)) {
                        echo '<div class="success"><p>Files directory is writeable!</div>';
                } else {
                        $errors = 1;
			echo '<div class="error"><p>Files directory is not not writeable!</div>';
                }
		clearstatcache();
		if($errors == 0) {
			?><p><button type="submit" name="cancel" value="next" onClick="loadStep('3');">Continue</button></p><?php
		} else {
			?><p>Please fix the errors above, then click retry.  <button type="submit" name="cancel" value="retry" class="del" onClick="loadStep('2');">Retry</button></p><?php
		}
	} // end step 2
	if($step == 3) {
		...
		<button type="submit" id="sql_submit" name="mysql_install" value="next">create config and tables</button>
		</form>
		...
		if(table_exists("responses",$dbname)) {
			$output = $output . '<div class="success"><p>Responses created!</p></div>';
		} else {
			$output = $output . '<div class="error"><p>Error creating responses table!</p></div>';
			$errors = 1;
		}
		if(table_exists("files",$dbname)) {
			$output = $output . '<div class="success"><p>Files table created!</p></div>';
		} else {
			$output = $output . '<div class="error"><p>Error creating files table!</p></div>';
			$errors = 1;
		}
		if($errors == 0) {
			echo $output;
			?><p><button type="submit" name="cancel" value="next" onClick="loadStep('5');">Continue</button></p><?php
		} else {
			echo $output;
			?><p><button type="submit" name="cancel" value="next" class="del" onClick="loadStep('3');">Retry</button></p><?php
		}
		} // end step 4
