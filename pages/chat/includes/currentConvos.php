...
while($row = mysql_fetch_array($result)) {
	if($row['status'] == "open") {
		if($row['answered'] > $row['updated']) {
			if((time() - $row['answered']) > $timeout) {
				mysql_query("UPDATE sessions SET status = 'closed', ended = '".time()."' WHERE id = '".$row['id']."' ");
				mysql_query("INSERT INTO transcript (name,message,convoID,class) VALUES ('System','Session has timed out, type a message to resume','".$row['id']."','notice') ");
			}
		}
	...
	if($row['status'] == "closed") {
		if( ((time() - $row['ended']) > $timeout_hide) && $row['hide'] != "yes") {
			mysql_query("UPDATE sessions SET hide = 'yes' WHERE id = '".$row['id']."' ");
			mysql_query("INSERT INTO transcript (name,message,convoID,class) VALUES ('System','Session has been ended due to inactivity.','".$row['id']."','notice') ");
		} else if($row['hide'] == "no") {
	...
	if($row['hide'] == "yes") {
		if((time() - $row['ended']) > $timeout_remove) {
			mysql_query("DELETE FROM transcript WHERE convoID = '".$row['convoID']."' ");
			mysql_query("DELETE FROM sessions WHERE id = '".$row['id']."' ");
		}
	}
$count = $count + 1;
}
