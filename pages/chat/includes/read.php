...
if(!empty($_GET['id'])) {
	include "base.php";
	$fetch = mysql_query("SELECT * FROM archive WHERE convoID = '".$_GET['id']."' ORDER BY id ASC");
	...
	while ($row = mysql_fetch_array($fetch)) {
                if($row['class'] == "notice") {
                echo '<li class="'. $row['class'] .'"><span class="user_said">' . $row['name'] . " dice :</span><br> " . $row['message'] . '</li>';
                } else {
                echo '<li class="'. $row['class'] .'"><span class="user_said">' . $row['time'] . " - " . $row['name'] . "
                dice :</span><br> " . $row['message'] . '</li>';
                }
        }
        echo "</ul></div>";
	?>
	...
	<input type="hidden" name="convoID" id="convoID" value="<?php echo $_GET['id'];?>">
