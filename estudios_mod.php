...
return edad;
	...
	function postulacionfinal(){
			...
			window.parent.fnCierraFormularioModificado(<?php echo $i; ?>,2);
	}


	function postulacionfinal_EVALUADOR2(){
			//alert('postular_final');
			CierraFormularioModificado_EVALUADOR2(<?php echo $i; ?>,2);
	}


	function CierraFormularioModificado_EVALUADOR2(data,origen)
{
            					...
            					//setTimeout(function(){ 
										var dataString = "id="+data+"&desde="+origen;
										 $.when(
											$.ajax({
											type: "GET",
											url: "pages/guarda_estado_mod9.php",
											data: dataString,
											async: false,
...
if(paso==4 || paso==3)
{
  ...
  if (window.console) console.log('--->pages/guarda_estudios_mod.php?'+$("#paso"+paso).serialize()+"&i="+llave+"&v="+vivienda);
  $.when(
  $.ajax({
		type: "GET",
		url: "pages/guarda_estudios_mod.php",
		data: dataString,
		async: false,
		success: function (data) {
		...
		error: function (objeto, quepaso, otroobj) {
		   if (window.console) console.log('ERROR AL INTENTAR ENVIAR  pages/guarda_estudios_mod.php --> '+objeto.responseText);
		}
	})
...
if(paso==1 || paso==2)
{
  ...
  if (window.console) console.log('--->pages/guarda_estudios_mod.php?'+$("#paso"+paso).serialize()+"&i="+llave);
  $.when(
  $.ajax({
		type: "GET",
		url: "pages/guarda_estudios_mod.php",
...
if(paso==4)
{
 ...
 if(!$("#condiciones").is(':checked')) 
 {
  ...
  if (window.console) console.log('--->pages/guarda_estudios_mod.php?'+$("#paso"+paso).serialize());
...
if(paso==3)
{
  ...
  if (window.console) console.log('--->pages/guarda_estudios_mod.php?'+dataString);
	...
	*/
	...
	$(".botonenviarpostulacion").click(function(){
		...
		if(valida_postulacion_contador(1)){  
			...
			$(".actionBar").append('<a href="#" class="postular_final" onclick="postulacionfinal_EVALUADOR2()" title="Felicitaciones, ahora puede postular." style="float: left; margin-left: 180px; background-color: #009933; color: white; ">CORREGIR POSTULACIÓN</a>');
		    <?php } ?>
		}
