...
$query = "select * from listar_datos_ponderacion_estudios WHERE ANTENSENA_POSTULANTE like '%MEDIA%'";
//echo $query;
...
echo "TOTAL DE ARCHIVOS".$numRows."<br>";
...
}else if (strpos($valor5,'MEDIA') !== false) { $pestudios = '60'; 
}else{ $pestudios = '0'; }


	$valorfinal = (0.6 * ((0.85 * $pnota) + (0.15 * $pestudios) )) + (0.4 * ((0.7 * $prenta) + (0.3 * $pgrupof)));
	//echo $valor."|".$valor2."|".$valor3."|".$valor5."|".$valor5."||".$prenta."|".$pestudios."|".$pgrupof."|".$pnota."|".round($valorfinal,0)."|".$row->IDPONDEDUCACION."<br>";
	
	$query = @"UPDATE
  PONDERACION_EDUCACION
SET
  ponderacion = ".round($valorfinal,0).",
  fecha_mod = NOW() 
WHERE
  IDPONDEDUCACION = ".$row->IDPONDEDUCACION.";<br>";
